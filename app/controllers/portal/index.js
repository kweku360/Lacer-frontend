...
export default Ember.Controller.extend({
        ...
        if(obj.meta.total == 0){
          ...
          for(var i=0;i<obj.suits.length;i++ ){
            if(obj.suits[i].suitstatus == "active" || obj.suits[i].suitstatus == "closed") {
              ...
              self.getSuitJudge( obj.suits[i].suitnumb);
  ...
  getSuitJudge : function(suitid){
    var self = this;
    var res = encodeURIComponent(suitid);
    $.ajax({
      //url: "http://localhost:8888/lacerapi/index.php/users/token",
      ...
      success: function(response) {
        ...
        console.log(obj);
        if(obj.meta.total == 0){
          self.set("suitInfo.suitJudge","None Set")
          self.pushSuit()
        }else{
          for(var i=0;i<obj.suitjudges.length;i++ ){
            self.set("suitInfo.suitJudge",obj.suitjudges[i].fullname)
            self.pushSuit()
          }
         }
      },
      error: function (model, response) {
        console.log("Unable to Retrieve Judges")
        self.pushSuit()
      }
    });
  },
  ...
  pushSuit : function(){
    var suitInfo  = {
      ...
      suitJudge :this.get("suitInfo.suitJudge"),
